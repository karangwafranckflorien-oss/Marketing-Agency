  ...
  return (
    <div className="fixed bottom-8 right-8 z-[60]">
          ...
          >
            ...
            <div className="p-8 border-b border-gray-100 bg-white/40 flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <div className="w-14 h-14 bg-[#0077FF] rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/20">
            ...
            <div className="p-8 bg-white/40 border-t border-gray-50">
              <div className="relative group">
                <input 
                  ...
                  className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-5 text-sm text-gray-900 placeholder:text-gray-300 focus:outline-none focus:border-[#0077FF] transition-all shadow-sm group-hover:shadow-md"
                />
                <button 
                  onClick={handleChat}
                  disabled={loading}
                  className="absolute right-3 top-1/2 -translate-y-1/2 w-12 h-12 bg-[#0077FF] rounded-xl flex items-center justify-center text-white hover:bg-gray-900 transition-all shadow-md"
                >
                  <Send size={18} />
                </button>
              </div>
      ...
      <motion.button
        ...
        className="w-20 h-20 bg-white border border-gray-100 shadow-2xl rounded-2xl flex items-center justify-center text-[#0077FF] relative group overflow-hidden"
      >
        <div className="absolute inset-0 bg-blue-50 opacity-0 group-hover:opacity-100 transition-opacity" />
        {isOpen ? <X size={32} /> : <MessageSquare size={32} className="relative z-10" />}
        {!isOpen && (
          <div className="absolute top-4 right-4 w-4 h-4 bg-[#FF7F00] rounded-full border-2 border-white z-20" />
        )}
      </motion.button>
    </div>
  );
